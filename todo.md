...
5. [ ] 이름 입력/초반 대사 회귀를 실제 화면 기준으로 계속 검증한다.
   ...
   - [ ] Part 2 저장 확인 compact 선택지는 ROM 슬롯을 `예/오` 배치로 갱신했지만, 기존 `ss0`는 선택지 VRAM/문자 캐시를 들고 있어 `마/오`처럼 보일 수 있음. 콜드 루트나 더 이른 상태에서 fresh 렌더 기준으로 재확인하고 필요 시 별도 tilemap hook을 추가한다.
