...
- [x] 레포 임시 캡처 추가 정리.
  ...
  - 현재 기준 전체 용량은 약 `196M`, `temp`는 약 `1.6M` 수준.
...
1. [ ] Part 2 튜토리얼을 끝까지 실제 화면 기준으로 검수한다.
   ...
   - [ ] 지형효과 비교 뒤 상태 목록/행동 복귀 흐름을 통과해 자주포/간접공격 설명 분기까지 실제 캡처로 재검증한다.
...
5. [ ] 이름 입력/초반 대사 회귀를 실제 화면 기준으로 계속 검증한다.
   ...
   - [x] Part 2 저장 확인 compact 선택지는 더 이른 상태에서 fresh 진행 기준 `예  오`로 정상 표시 확인.
