...
- [x] iter18: **두 영어 잔존 BG 모두 한글화 완료** — 작전선택(레드스타/블루문/그린어스/코멧/옐로) + 전략오버맵(레드스타 궁전/공장/궁전, 8bpp). 자동진행 깊은화면 전부 한글 정상. 전 게이트 PASS.
- [ ] (선택) 하단 오벌 국가명 보강(현 '궁전'만 — 필기체 판독불가, 사용자 국가명 제공 시 추가) / ﾆﾙ 무탄약 표시 / 결과·엔딩 near-end 세이브.
