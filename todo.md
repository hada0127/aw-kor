...
- [ ] 예/아니오 compact 화면·이름확인·저장확인·결과/스코어 화면 fresh-boot 표본 검증
- [ ] **모드선택 월드맵 배경 영어 국가명** fresh-boot로 실 ROM 잔존 여부 확정(추정: stale savestate)
- [ ] **1+2 선택 화면 로고 "™" 잔존** 처리 판단(제거 또는 의도적 보존 합의)
- [ ] Phase B 부호 복원 후 박스 오버런/줄넘침 회귀 픽셀 실측
