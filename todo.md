...
6. [ ] 튜토리얼을 통과한 뒤 Part 2 본편 캠페인/전투 화면 전체로 확장한다.
   ...
   - 2026-06-07 Part 2 튜토리얼 잔여 직접 패치 문장 재검수: 확장 스윕에서 오래된 VRAM 잔상처럼 보였던 `해녀` 후보는 현재 ROM/소스에 없고 실제 문구는 `해냈어`였다. 추가로 직접 패치 행에서 남아 있던 붙은 표기 `0xD99C41` `확실히보일거야`, `0xD9A208` `은공중유닛이야`를 각각 `확실히 보일 거야`, `은 공중 담당이야`로 보정했다.
   - 현재 ROM 인코딩 검색에서 `확실히보일거야`, `은공중유닛이야`, `해녀`, `내릴곳`, `다리앞`, `어기까지`, `십자버튼`은 0건이고, 새 문구는 `0xD99C41`, `0xD9A208`에 들어갔다. `해냈어`, `내릴 곳`, `다리 앞`, `십자 버튼`은 각각 기대 주소에서 확인했다.
   - 이번 변경 후 재빌드와 산출물 동기화, preview BPS/IPS 재생성 및 round-trip을 완료했다. `py_compile`, `qa_text_fit.py`(`written=19053`, `level0=19051`, `level2=2`, `level3=0`, `level4=0`, `overflow=0`, `no_ko=0`, `visual-wider=17`), `qa_japanese_residuals.py --min-score 13` candidate 0, `qa_placeholder_residuals.py`(`rom_placeholder_hits=0`), `phase6_basic_test.py` full/final/title_test 통과. 세 산출물 SHA-256은 `7bf2a588255dcd62f9721ff3f5daa4a6ca2e15dfddc862036b4673362c3c014f`, preview BPS/IPS SHA-256은 각각 `34953f10090c98faa72fecb171735c176c611c94cd74a4f58f3445b4a6b12052`, `860e0c8e0b5e679829bd51d608b598d46411c29fcf88c4921c52b2e357bb2e6b`이다.
