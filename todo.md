...
- [x] Part 2 첫 전투 튜토리얼 수송차 설명의 일본어 잔여 제거.
  ...
  - 원본 조립 순서가 주소 순서와 달라 겹치던 중간 코멘트를 직접 비우고, 실제 캡처 `temp/p2_apc_text_verify5/sheet.png` 기준 일본어 원문 잔여가 사라진 것을 확인.
- [ ] Part 2 첫 전투 수송차/탑승 설명 문장 조립 재구성 및 후속 진행 검증.
  - 현재 수송차 설명은 원문 잔여는 없지만 문장 순서가 투박하므로, 다음 진행에서 해당 스크립트 조립 순서를 더 크게 재구성해 자연스럽게 만든다.
