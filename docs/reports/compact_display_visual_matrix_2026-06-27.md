...
- `data/compact_display_read_watch_b84_power_titles_coid_current.json`: current=False groups=['b84_compact_power_display_overrides'] cases=11 hits=11 direct_reads=11 targets=11 proof_mode=emulator_read_watch_current_rom_live_ram_coid_variant
...
- A2 CO power names now have 0/36 synthetic RAM-field source proof plus one current representative profile screen, but not natural-route 36 per-power screen captures.
