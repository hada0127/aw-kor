# Game Wars 한글화 프로젝트 - 실행 계획서

**프로젝트명**: Game Boy Advance Game Wars (1+2) 완전 한글화  
**목표**: 일본어판 ROM을 100% 한글화하여 완성된 패치 배포  
**예상 기간**: 7~18개월 (팀 규모에 따라)

---

## PHASE 1: 준비 및 환경 구축 (1-2주)

### 1-1. 도구 설치 및 설정
- [ ] VisualBoyAdvance M v2.1.4 다운로드 및 설치
- [ ] HxD Hex 편집기 설치
- [ ] Crystal Tile 2.5 설치 (GitHub에서)
- [ ] Python 3.9+ 설치
- [ ] Git 설치 및 리포지토리 초기화
- [ ] VS Code 설치 (권장)

### 1-2. 개발 환경 구성
- [ ] GitHub/GitLab 리포지토리 생성
- [ ] project/original 폴더에 Game Wars 1+2 ROM 배치
- [ ] ROM 파일 검증 (파일 크기, 체크섬)
- [x] 프로젝트 디렉토리 구조 설계
  ```
  aw-kor/
  ├── original/          (원본 롬)
  ├── docs/              (문서)
  ├── tools/             (도구 및 스크립트)
  ├── data/              (추출된 텍스트)
  ├── translation/       (번역 파일)
  ├── output/            (생성된 롬)
  ├── .gitignore
  └── README.md
  ```
- [x] .gitignore 설정 (ROM 파일 제외)

### 1-3. 커뮤니티 조사 및 연락
- [ ] 한글로게임 방문 및 Game Wars 정보 확인
- [ ] RomHacking.net 계정 생성
- [ ] GBAtemp 포럼 가입
- [ ] 네이버 카페 "한글화하는 사람들" 가입
- [ ] 기존 GBA 한글화 팀 연락 (정보 공유)

---

## PHASE 2: ROM 분석 (2-4주)

### 2-1. 게임 플레이 및 분석
- [ ] Game Wars 1 전체 플레이
  - [ ] 모든 스토리 모드 클리어
  - [ ] 모든 메뉴 확인
  - [ ] 모든 유닛/아이템 확인
  - [ ] 텍스트량 추정
- [ ] Game Wars 2 전체 플레이 (동일)
- [ ] UI 구조 문서화
- [ ] 메뉴 흐름도 작성

### 2-2. ROM 구조 분석
- [ ] HxD에서 ROM 열기
- [ ] 게임 헤더 정보 읽기
  - [ ] 게임 제목
  - [ ] 회사 코드
  - [ ] ROM 크기
  - [ ] 체크섬
- [ ] 텍스트 데이터 위치 특정
  - [ ] Hex 에디터에서 일본어 문자 검색
  - [ ] 패턴 분석
  - [ ] 대략적 위치 매핑

### 2-3. 에뮬레이터 디버깅
- [ ] VisualBoyAdvance M에서 ROM 열기
- [ ] 메모리 뷰어 사용
- [ ] 게임 실행 중 메모리 추적
- [ ] 텍스트 저장 위치 확인

### 2-4. 분석 문서 작성
- [ ] ROM 구조 다이어그램
- [ ] 주요 주소 매핑 테이블
- [ ] 텍스트 데이터 영역 표시
- [ ] 포인터 위치 리스트

---

## PHASE 3: 텍스트 추출 (2-4주)

### 3-1. 인코딩 분석 및 .tbl 파일 생성
- [ ] 게임의 문자 인코딩 결정
  - [ ] Shift-JIS 확인
  - [ ] 특수 문자 확인
- [ ] RSEARCH 도구 실행
- [ ] 상대적 검색으로 패턴 추출
- [ ] 초안 .tbl 파일 작성
- [ ] HxD에서 .tbl 로드 및 검증
- [ ] 수동 수정 및 최종화
- [ ] .tbl 파일 git에 커밋

### 3-2. 전체 텍스트 추출
- [ ] Python 스크립트로 자동 추출
  ```python
  # tools/extract_text.py
  - ROM 읽기
  - .tbl 파일 로드
  - 모든 텍스트 구간 추출
  - CSV 형식 저장
  ```
- [ ] Game Wars 1 전체 텍스트 추출
  - [ ] 메인 텍스트
  - [ ] UI 텍스트
  - [ ] 유닛/아이템 이름
- [ ] Game Wars 2 전체 텍스트 추출 (동일)
- [ ] 텍스트 정렬 및 정제
  - [ ] 중복 제거
  - [ ] 위치 기준 정렬
  - [ ] 메타데이터 추가 (컨텍스트, 길이)

### 3-3. 포인터 매핑
- [ ] 모든 텍스트 포인터 위치 식별
- [ ] 포인터 테이블 생성
  ```
  주소       | 포인터값 | 텍스트
  0x100000   | 0x200000 | "あなたの名前"
  0x100004   | 0x200010 | "おはようございます"
  ```
- [ ] 포인터 테이블 검증

### 3-4. 데이터 정리
- [ ] CSV 파일로 정렬 (tools/data/game_wars_1_text.csv)
  ```
  address,pointer_addr,japanese,characters,context,notes
  0x100000,0x100010,"あなたの名前",6,"캐릭터 이름 입력",메인 캐릭터
  ```
- [ ] 스프레드시트로 공유 준비
- [ ] 번역팀 공유 용 최종 데이터 세트

---

## PHASE 4: 번역 진행 (6-12개월)

### 4-1. 번역팀 구성
- [ ] 리더 모집
  - [ ] 게임 스토리 이해
  - [ ] 톤앤매너 일관성
- [ ] 번역가 모집
  - [ ] 일본어-한글 능력
  - [ ] 게임 번역 경험
- [ ] 검수자 모집
  - [ ] 네이티브 한글 화자
  - [ ] 기술 검수 담당자
- [ ] 리드 설정 및 역할 분담

### 4-2. 번역 우선순위 설정
- [ ] Priority 1: UI/메뉴 (게임플레이 필수)
  - [ ] 메뉴 항목
  - [ ] 상태창
  - [ ] 버튼 레이블
- [ ] Priority 2: 메인 스토리
  - [ ] 캐릭터 대사
  - [ ] 전투 대사
  - [ ] 시스템 메시지
- [ ] Priority 3: 추가 콘텐츠
  - [ ] 튜토리얼
  - [ ] 도움말
  - [ ] 아이템 설명

### 4-3. 번역 진행 관리
- [ ] Google Sheets 또는 협업 도구로 진행 추적
- [ ] 주간 진행률 체크
- [ ] 번역 일관성 관리 (용어집 작성)
  ```
  용어 | 한글 | 일본어 | 참고
  HP   | 체력 | ヒットポイント | 게임 전반
  MP   | 마나 | マジックポイント | 게임 전반
  ```
- [ ] 번역 갈등 해결 프로세스

### 4-4. 검수 및 수정
- [ ] 번역 완료된 텍스트 검수
- [ ] 길이 조정 (화면 overflow 방지)
- [ ] 톤앤매너 일관성 확인
- [ ] 오류 수정 및 최종 승인

---

## PHASE 5: 기술적 구현 (4-8주)

### 5-1. 자동화 빌드 시스템 구축
- [x] Python 스크립트 개발
  - [x] `extract_text.py`: 텍스트 추출
  - [x] `import_text.py`: 번역 텍스트 가져오기
  - [x] `update_pointers.py`: 포인터 자동 업데이트
  - [x] `build_rom.py`: 최종 ROM 생성
- [ ] Batch 빌드 스크립트 작성
  ```batch
  @echo off
  python tools/extract_text.py
  python tools/import_text.py
  python tools/update_pointers.py
  python tools/build_rom.py output.gba
  ```
- [ ] 빌드 프로세스 테스트 (텍스트 추출 완료 후)

### 5-2. 포인터 업데이트 시스템
- [x] 포인터 계산 로직 구현 (update_pointers.py)
  ```python
  def calculate_new_pointers(text_data):
      # 한글 텍스트로 인한 길이 변화 계산
      # 모든 포인터 재계산
      # 바이너리 형식으로 변환 (리틀엔디안)
  ```
- [x] 포인터 재배치 알고리즘 구현
- [ ] 자동 검증 로직 추가 (테스트 데이터 필요)

### 5-3. 한글 폰트 데이터 준비
- [ ] 폰트 분석
  - [ ] 기존 폰트 데이터 위치 식별
  - [ ] 글리프(glyph) 구조 이해
- [ ] 한글 폰트 생성/수정
  - [ ] 기본 한글 2,350자 글리프 생성
  - [ ] 폰트 데이터 형식 변환
- [ ] ROM에 삽입하기 위해 폰트 데이터 준비
- [ ] 폰트 크기 검증 (ROM 크기 내)

### 5-4. 번역 텍스트 삽입
- [ ] 번역 CSV를 바이너리로 변환
  ```python
  def text_to_binary(text, encoding='euc-kr'):
      return text.encode(encoding)
  ```
- [ ] ROM의 텍스트 영역에 삽입
- [ ] 바이너리 길이 검증
- [ ] 인코딩 검증

### 5-5. ROM 최종화
- [ ] ROM 크기 확인 (32MB 이내)
- [ ] 체크섬 재계산
- [ ] ROM 헤더 정보 업데이트
- [ ] 최종 ROM 저장

---

## PHASE 6: QA 및 테스트 (4-12주)

### 6-1. 기본 기능 테스트
- [ ] 에뮬레이터에서 ROM 실행
- [ ] 게임 시작 화면 확인
- [ ] 메뉴 네비게이션 테스트
- [ ] 저장/로드 테스트
- [ ] 게임 진행성 테스트

### 6-2. 텍스트 표시 검증
- [ ] 모든 한글 텍스트 표시 확인
- [ ] 텍스트 overflow 확인
  - [ ] 메뉴 아이템 길이
  - [ ] 대사 상자 너비
  - [ ] 상태창 공간
- [ ] 폰트 렌더링 품질 확인
- [ ] 특수 문자 표시 확인

### 6-3. 게임플레이 완전 검증
- [ ] Game Wars 1 전체 플레이
  - [ ] 모든 스토리 모드 진행
  - [ ] 모든 전투 확인
  - [ ] 모든 메뉴 접근
  - [ ] 모든 아이템/유닛 확인
- [ ] Game Wars 2 전체 플레이 (동일)
- [ ] 난이도별 플레이 (쉬움, 보통, 어려움)
- [ ] 버그/오류 로그 작성

### 6-4. 버그 추적 및 기록
- [ ] 버그 추적 시스템 설정 (GitHub Issues)
- [ ] 발견된 버그 분류
  - [ ] Critical (게임 진행 불가)
  - [ ] Major (기능 오류)
  - [ ] Minor (표시 오류, 오타)
  - [ ] Enhancement (개선)
- [ ] 버그별 우선순위 설정

### 6-5. 수정 및 재테스트
- [ ] Critical 버그 수정
- [ ] Major 버그 수정
- [ ] Minor 버그 수정
- [ ] 각 수정마다 ROM 재빌드
- [ ] 재테스트 (회귀 테스트)

### 6-6. 최종 검수
- [ ] 최종 ROM 검증
- [ ] 전체 텍스트 재확인
- [ ] 성능 테스트 (프레임률, 속도)
- [ ] 에뮬레이터 호환성 테스트
  - [ ] VisualBoyAdvance M
  - [ ] mGBA
  - [ ] VBA-M

---

## PHASE 7: 배포 및 커뮤니티 (1-2주)

### 7-1. 배포 준비
- [ ] 최종 패치 파일 생성
- [ ] README 작성
  - [ ] 설치 방법
  - [ ] 사용법
  - [ ] 알려진 문제
  - [ ] 기여자 목록
- [ ] 라이선스 결정 (GPL, MIT 등)
- [ ] GitHub Release 생성
- [ ] 패치 파일 호스팅

### 7-2. 커뮤니티 공유
- [ ] 한글로게임에 패치 등록
- [ ] RomHacking.net에 등록 (영문 버전)
- [ ] GBAtemp에 공지
- [ ] 네이버 카페에 공유
- [ ] GitHub 링크 배포

### 7-3. 피드백 수집
- [ ] GitHub Issues로 버그 보고 받기
- [ ] 커뮤니티 피드백 검토
- [ ] 후속 패치 계획
- [ ] 추가 개선 사항 목록화

---

## 마일스톤별 체크포인트

| 마일스톤 | 목표 | 예상 기간 | 상태 |
|---------|-----|---------|------|
| M1: 환경 구축 | Phase 1 완료 | 1-2주 | ◐ (부분 완료) |
| M2: ROM 분석 | Phase 2 완료, 구조 파악 | 2-4주 | ☐ |
| M3: 텍스트 추출 | Phase 3 완료, 전체 텍스트 추출 | 2-4주 | ☐ |
| M4: 번역 50% | 우선순위 텍스트 번역 완료 | 3-6개월 | ☐ |
| M5: 번역 100% | 모든 텍스트 번역 완료 | 6-12개월 | ☐ |
| M6: 베타 ROM | 자동화 시스템 구축, 첫 ROM 생성 | 1-2주 | ☐ |
| M7: Alpha 테스트 | 내부 테스트 완료 | 2-4주 | ☐ |
| M8: Beta 공개 | 커뮤니티 베타 테스트 | 2-4주 | ☐ |
| M9: 최종 버전 | 최종 ROM 및 패치 배포 | 1주 | ☐ |

---

## 리스크 관리

### 식별된 리스크

| 리스크 | 확률 | 영향 | 대응 방안 |
|------|------|------|---------|
| ROM 구조 복잡도 | 중 | 높음 | 커뮤니티와 상담, 기존 프로젝트 참고 |
| 번역팀 이탈 | 중 | 높음 | 여러 번역가 확보, 진행률 공개 |
| 기술적 오류 | 중 | 높음 | 자동화 시스템, 버전 관리, 테스트 철저 |
| ROM 크기 초과 | 낮음 | 높음 | 텍스트 압축, 폰트 최적화 |
| 호환성 문제 | 중 | 중간 | 여러 에뮬레이터 테스트 |

---

## 리소스 요구사항

### 필수 인원
- **프로젝트 리드**: 1명 (기술, 관리)
- **번역가**: 2-3명 (일본어-한글)
- **검수자**: 1-2명 (품질 관리)
- **기술 지원**: 1명 (ROM 해킹, 자동화)
- **QA**: 1-2명 (테스트, 버그 추적)

### 필수 도구 (모두 무료)
- VisualBoyAdvance M
- HxD Hex Editor
- Crystal Tile 2.5
- Python 3.9+
- Git
- Google Sheets (협업)

### 예상 비용
**0원** (모든 도구가 무료 오픈소스)

---

## 성공 기준

✓ 게임이 완벽하게 한글로 표시됨  
✓ 모든 텍스트가 자연스러운 한글  
✓ 게임플레이에 오류나 버그 없음  
✓ 모든 에뮬레이터에서 호환됨  
✓ 커뮤니티에서 긍정적 평가  

---

## 다음 단계

### 현재 상태 (2026-05-11)

✓ **완료된 작업:**
- 프로젝트 폴더 구조 설정
- .gitignore 및 README.md 작성
- Python 자동화 스크립트 작성 (extract, import, update, build)
- 기술 문서 작성 (ROM 분석, 번역 프로세스 가이드)

⏳ **필요한 수동 작업:**
1. **즉시 (이번 주)**
   - [ ] VisualBoyAdvance M v2.1.4 설치
   - [ ] HxD Hex 에디터 설치
   - [ ] Crystal Tile 2.5 설치
   - [ ] Python 3.9+ 설치
   - [ ] Game Wars 1+2 ROM 파일 준비 (original/ 폴더에 배치)

2. **1주 내** (PHASE 2: ROM 분석 시작)
   - [ ] VisualBoyAdvance에서 게임 실행 확인
   - [ ] 기본 ROM 분석 시작
   - [ ] 텍스트 위치 파악

3. **2주 내** (PHASE 3 준비)
   - [ ] ROM 구조 분석 완료
   - [ ] tools/game_wars.tbl 파일 작성/수정
   - [ ] data/pointers.csv 작성

4. **4주 내** (PHASE 4 시작)
   - [ ] python tools/extract_text.py 실행
   - [ ] 전체 텍스트 추출 완료 (data/game_wars_1_text.csv)
   - [ ] 번역팀 구성 시작
   - [ ] Google Sheets 설정 (번역 공유)

**참고**: 현재 ROM 분석 도구와 ROM 파일 없이는 PHASE 2 이후 진행 불가

---

**문서 작성일**: 2026년 5월 11일  
**마지막 수정**: 2026년 5월 11일  
**버전**: 1.0
