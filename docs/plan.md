...
- [ ] Python 스크립트로 자동 추출
  ...
  - CSV 형식 저장
  ```
- [ ] Game Wars 1 전체 텍스트 추출
  - [ ] 메인 텍스트
  - [ ] UI 텍스트
  - [ ] 유닛/아이템 이름
- [ ] Game Wars 2 전체 텍스트 추출 (동일)
- [ ] 텍스트 정렬 및 정제
  - [ ] 중복 제거
  - [ ] 위치 기준 정렬
  - [ ] 메타데이터 추가 (컨텍스트, 길이)
...
- [ ] 포인터 테이블 생성
  ```
  주소       | 포인터값 | 텍스트
  0x100000   | 0x200000 | "あなたの名前"
  0x100004   | 0x200010 | "おはようございます"
  ```
- [ ] 포인터 테이블 검증

### 3-4. 데이터 정리
- [ ] CSV 파일로 정렬 (tools/data/game_wars_1_text.csv)
  ```
...
| M2: ROM 분석 | Phase 2 완료, 구조 파악 | 2-4주 | ☐ (도구/ROM 대기) |
| M3: 텍스트 추출 | Phase 3 완료, 전체 텍스트 추출 | 2-4주 | ☐ (M2 의존) |
...
⏳ **필요한 수동 작업:**
1. **즉시 (이번 주)**
   - [ ] VisualBoyAdvance M v2.1.4 설치
   - [ ] HxD Hex 에디터 설치
   - [ ] Crystal Tile 2.5 설치
   - [ ] Python 3.9+ 설치
   - [ ] Game Wars 1+2 ROM 파일 준비 (original/ 폴더에 배치)

2. **1주 내** (PHASE 2: ROM 분석 시작)
   - [ ] VisualBoyAdvance에서 게임 실행 확인
   - [ ] 기본 ROM 분석 시작
   - [ ] 텍스트 위치 파악

3. **2주 내** (PHASE 3 준비)
   - [ ] ROM 구조 분석 완료
   - [ ] tools/game_wars.tbl 파일 작성/수정
   - [ ] data/pointers.csv 작성

4. **4주 내** (PHASE 4 시작)
   - [ ] python tools/extract_text.py 실행
   - [ ] 전체 텍스트 추출 완료 (data/game_wars_1_text.csv)
   - [ ] 번역팀 구성 시작
   - [ ] Google Sheets 설정 (번역 공유)

**참고**: 현재 ROM 분석 도구와 ROM 파일 없이는 PHASE 2 이후 진행 불가
