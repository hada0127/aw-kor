...
## 2026-05-23 19:00 — v45 좌측 그리드 알파벳 완성

### ✅ 완료
- 그리드 두 영역 RE 완료 (좌측=top/bottom 슬롯, 우측=top_extra/bot_extra)
- 이분탐색으로 좌측 메인 슬롯 범위 발견: 128-191 (첫 32 셀)
- v45: A-Z + 0-9 알파벳 8x16 글리프로 가독성 좋게 표시
- 한글 dialog 보존

### ⏳ 진행 중
- 모드 토글 (L/R 버튼) — 다른 페이지가 있는지 확인
- 셀 → 저장 매핑 RE: 사용자가 그리드 셀 선택 시 어떤 SJIS/ASCII가 저장되는지
- 다음 화면에서 저장된 이름 출력 검증

### 다음 단계
1. mGBA Lua로 그리드에서 셀 선택 (A 키) 시 어떤 메모리 위치에 어떤 값이 쓰이는지 watchpoint
