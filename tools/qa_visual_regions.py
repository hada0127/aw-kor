...
PART1_OPTION_GOLDEN_LAYER_SHA256 = {
    # Regression guard for the 2026-07-07 custom 통/룸 work: 대전 is the
    # unchanged control label and must stay byte-identical to the prior render.
...
def run_asset_checks() -> list[str]:
    ...
    for name, _off, text, max_size in th.PART1_MODE_OPTION_BLOCKS:
        try:
            ...
            golden_sha = PART1_OPTION_GOLDEN_LAYER_SHA256.get(name)
            if golden_sha is not None:
                actual_sha = hashlib.sha256(layer.tobytes()).hexdigest()
                if actual_sha != golden_sha:
                    raise AssertionError(
                        f"part1 option {name}: golden layer hash changed {actual_sha} != {golden_sha}"
                    )
